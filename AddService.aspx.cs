    ...
    public partial class AddService : System.Web.UI.Page
    {
        ...
        protected void btnSave_ServerClick(object sender, EventArgs e)
        {
            try
            {
                ...
                string sp = "SP_INSERT_SERVICE";

                if (Request["id"] != null)
                {
                    param[40] = new SqlParameter("@Patient_Id", Request["id"].ToString());
                    sp = "SP_UPDATE_SERVICE";
                }
                ...
                if (_result > 0)
                {
                    ...
                    SaveReportRendred(_result);
                    if (Request["id"] == null)
                    {
                        ViewState["Id"] = _result;
                        btnPrintForm.Attributes.Add("style", "display:inline");
                        btnPrintReport.Attributes.Add("style", "display:inline");
                    }
        ...
        public void SaveReportRendred(int ServiceId)
        {
            try
            {
                DataTable dt = new DataTable();
                ...
                db.executeSP("SP_DELETE_SERVICE_RENDERED", param);
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        param[5] = new SqlParameter("@ServiceId", ServiceId);

                        db.executeSP("SP_INSERT_SERVICE_RENDERED", param);
                    }
                }
        ...
        {
            PrintRequest(ViewState["Id"].ToString(), "r");
        }

        protected void btnPrintForm_ServerClick(object sender, EventArgs e)
        {
            PrintRequest(ViewState["Id"].ToString(), "f");
        }
    }
