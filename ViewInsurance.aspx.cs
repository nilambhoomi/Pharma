    ...
    public partial class ViewInsurance : System.Web.UI.Page
    {
        ...
        protected void btnSearch_ServerClick(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(txtSearch.Value))
            {
                ...
                string query = " where REPLACE(REPLACE(Name, CHAR(13), ''), CHAR(10), '') like '%" + txtSearch.Value + "%' or Address like '%" + txtSearch.Value + "%' ";
        ...
        protected void BindGrid(string query, int pageindex, int pagesize = 25, string columnname = "Id", string sortorder = "desc")
        {
            ...
            try
            {
                ...
                {
                    SqlCommand gComm = new SqlCommand("SP_INSURANCE_INDEX_DATA", gConn)
                    {
        ...
        public void deleteRecord(string id)
        {
            try
            {
                string _query = " delete from tblInsuranceComp where Id=" + id;
        ...
        [WebMethod]
        public static string[] getInsuranceName(string prefix)
        {
            ...
            DataSet ds = db.selectData("select Name from tblInsuranceComp where REPLACE(REPLACE(Name, CHAR(13), ''), CHAR(10), '') like '%" + prefix + "%'");
            if (ds.Tables[0].Rows.Count > 0)
            {
                string cmpname = "";
                for (int i = 0; i <= ds.Tables[0].Rows.Count - 1; i++)
                {
                    cmpname = ds.Tables[0].Rows[i]["Name"].ToString();
